...
var (
	userDBTypes = map[string]string{`ID`: `bigint`, `Username`: `character varying`, `Email`: `character varying`, `Password`: `character varying`, `CreatedAt`: `timestamp with time zone`, `LastLogin`: `timestamp with time zone`}
	_           = bytes.MinRead
)
